...
func (bi *BackingImage) prepareFromSync(headInitiator *nvme.Initiator, fromAddress, srcLvsUUID, srcBackingImageName string, encryption types.EncryptionType, credential map[string]string) (err error) {
	...
	// encryption == encrypt
	// writeZero := true
	// target should setup the encrypted device, need to format it first

	// encryption == decrypt
	// writeZero := false
	// source should setup the encrypted device, no need to format
	...
	// openTarget
	// if encrypt
	// 1. setup crypto device on initiator.Endpoint
	// 2. open crypto device at /dev/mapper/....
	// if decrypt or ignore
	// 1. directly open initiator.Endpoint
	// defer
	// close file handler
	// close crypto device if encrypt
	// stop initiator
	...
	defer func() {
		...
		bi.log.Info("Stopping haed NVMe initiator")
		if _, opErr := headInitiator.Stop(true, true, false); opErr != nil {
			bi.log.WithError(opErr).Errorf("Failed to stop the backing image head NVMe initiator")
		}
	}()
	...
	// openSource
	// if decrypt
	// 1. setup crypto device on source initiator endpoint
	// 2. open crypto device at /dev/mapper/....
	// if encrypt or ignore
	// 1. directly open initiator.Endpoint
	// defer
	// close file handler
	// close crypto device if decrypt
	// stop initiator
