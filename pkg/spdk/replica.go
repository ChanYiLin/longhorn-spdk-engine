...
func ServiceReplicaToProtoReplica(r *Replica) *spdkrpc.Replica {
	...
	if r.BackingImage != nil {
		...
		if err != nil {
			logrus.Errorf("Failed to extract backing image name from %v: %v", r.BackingImage.Name, err)
		}
...
func (r *Replica) RebuildingDstSnapshotRevert(spdkClient *spdkclient.Client, snapshotName string) (dstRebuildingLvolAddress string, err error) {
	...
	if snapshotName != "" {
		...
		if IsBackingImageSnapLvolName(snapshotName) {
			...
			// On the current node, this backing image snapshot name is not the same due to the different LVS UUID.
